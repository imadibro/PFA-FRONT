...
const columns = ({ toggleEditMode, deleteObject }: IActionColumnsProps<IOperation>): GridColDef[] => {
  return [
    ...
    },
    {
      ...
      flex: 1,
      renderCell: ({ row }: ICellType<IOperation>) => (
        <Typography noWrap sx={{ fontWeight: 500, color: 'text.secondary' }} title={row.clientAbri}>
          {row.clientAbri ? row.clientAbri : '-'}
        </Typography>
      )
    },
    {
      ...
      renderCell: ({ row }: ICellType<IOperation>) => (
        <Typography noWrap sx={{ fontWeight: 500, color: 'text.secondary' }}>
          {row.site ? `${row.site.map(s => s.label)} - ${row.site.map(s => s.siteNbr)}` : '-'}
        </Typography>
      )
    },
    {
